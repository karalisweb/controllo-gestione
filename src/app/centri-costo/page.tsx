...
import { Plus, Edit2, Trash2, Search, TrendingDown, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react";
import { toast } from "sonner";
import { formatCurrency } from "@/lib/utils/currency";
import type { CostCenter } from "@/types";

type CostCenterWithStats = CostCenter & {
  totalExpected?: number;
  spent?: number;
  expectedExpensesCount?: number;
};

type SortKey = "name" | "voci" | "budget" | "spent" | "remaining";
...
export default function CentriCostoPage() {
  const [centers, setCenters] = useState<CostCenterWithStats[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/cost-centers?year=2026");
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      switch (sortKey) {
        ...
        case "voci": return compareNumbers(a.expectedExpensesCount, b.expectedExpensesCount, sortDir);
        case "budget": return compareNumbers(a.totalExpected, b.totalExpected, sortDir);
        case "spent": return compareNumbers(a.spent, b.spent, sortDir);
        case "remaining": return compareNumbers((a.totalExpected || 0) - (a.spent || 0), (b.totalExpected || 0) - (b.spent || 0), sortDir);
        default: return 0;
      }
    });
  ...
  const totalSpent = centers.reduce((s, c) => s + (c.spent || 0), 0);
  const totalVoci = centers.reduce((s, c) => s + (c.expectedExpensesCount || 0), 0);

  const handleCreate = async (data: Partial<CostCenter>) => {
  ...
  const handleUpdate = async (data: Partial<CostCenter>) => {
    if (!editing) return;
    const res = await fetch(`/api/cost-centers/${editing.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
  ...
  const handleDelete = async (c: CostCenter) => {
    if (!confirm(`Eliminare il centro di costo "${c.name}"? Le spese collegate non vengono eliminate ma perderanno l'assegnazione.`)) return;
  ...
  if (loading) {
    return (
      <div className="min-h-screen">
        <MobileHeader title="Centri di Costo" />
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="p-3 sm:p-4 lg:p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground">Centri di Costo</h1>
            <p className="text-xs sm:text-sm text-muted-foreground">
              Categorie con cui aggreghi le tue spese (Telefonia, Software, Server, Stipendi, ecc.).
            </p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4">
          <Card className="p-3 sm:p-4">
            ...
            <p className="text-xs text-muted-foreground">{totalVoci} spese collegate</p>
          </Card>
          <Card className="p-3 sm:p-4">
            <p className="text-xs text-muted-foreground mb-1">Budget annuo</p>
            <p className="text-base sm:text-xl font-bold font-mono text-red-500 dark:text-red-400">{formatCurrency(totalExpected)}</p>
          </Card>
          <Card className="p-3 sm:p-4">
            <p className="text-xs text-muted-foreground mb-1">Speso</p>
            <p className="text-base sm:text-xl font-bold font-mono text-foreground">{formatCurrency(totalSpent)}</p>
          </Card>
        </div>
            ...
            <CardContent className="p-8 text-center text-muted-foreground">
              {centers.length === 0 ? (
                <>
                  ...
                  <p className="font-medium mb-1">Nessun centro di costo</p>
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                        <span>{c.expectedExpensesCount || 0} voci</span>
                        <span>•</span>
                        <span className="font-mono text-red-500">{formatCurrency(c.totalExpected || 0)}</span>
                      </div>
                    </div>
                  </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right cursor-pointer select-none hover:bg-muted/50" onClick={() => handleSort("budget")}>
                      <span className="inline-flex items-center gap-1">Budget annuo <SortIcon col="budget" /></span>
                    </TableHead>
                    <TableHead className="text-right cursor-pointer select-none hover:bg-muted/50" onClick={() => handleSort("spent")}>
                      <span className="inline-flex items-center gap-1">Speso <SortIcon col="spent" /></span>
                    </TableHead>
                    <TableHead className="text-right cursor-pointer select-none hover:bg-muted/50" onClick={() => handleSort("remaining")}>
                      <span className="inline-flex items-center gap-1">Rimanente <SortIcon col="remaining" /></span>
                    </TableHead>
                ...
                <TableBody>
                  {filtered.map((c) => {
                    const remaining = (c.totalExpected || 0) - (c.spent || 0);
                    return (
                      <TableRow key={c.id}>
                        ...
                        <TableCell className="text-center"><Badge variant="secondary">{c.expectedExpensesCount || 0}</Badge></TableCell>
                        <TableCell className="text-right font-mono text-red-600">{formatCurrency(c.totalExpected || 0)}</TableCell>
                        <TableCell className="text-right font-mono">{formatCurrency(c.spent || 0)}</TableCell>
                        <TableCell className={`text-right font-mono font-bold ${remaining >= 0 ? "text-green-600" : "text-red-600"}`}>{formatCurrency(remaining)}</TableCell>
                  ...
                  <TableRow className="bg-muted/50 font-bold">
                    ...
                    <TableCell className="text-right font-mono text-red-600">{formatCurrency(totalExpected)}</TableCell>
                    <TableCell className="text-right font-mono">{formatCurrency(totalSpent)}</TableCell>
                    <TableCell className={`text-right font-mono ${totalExpected - totalSpent >= 0 ? "text-green-600" : "text-red-600"}`}>{formatCurrency(totalExpected - totalSpent)}</TableCell>
                    <TableCell></TableCell>
                  </TableRow>
                </TableBody>
              </Table>
