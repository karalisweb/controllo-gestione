...
L'interfaccia trasmette un senso di **professionalità**, **pulizia** e **modernità**, con ampio uso di whitespace e contrasto elevato tra sfondi scuri e testo chiaro.
...
### 2.6 Colori Contenuti Derivati

| Tipo contenuto               | Colore      |
|------------------------------|-------------|
| LinkedIn Post (Human)        | `#22c55e`   |
| LinkedIn Post (Technical)    | `#d4a726`   |
| LinkedIn Post (Commercial)   | `#a855f7`   |
| Newsletter (LinkedIn/Substack)| `#3b82f6`  |
| Substack Note                | `#eab308`   |
| Immagine                     | `#d4a726`   |
...
> **Nota**: Content HUB attualmente usa dimensioni intermedie come `0.85rem`, `0.9rem`, `1.1rem`. Queste vanno allineate alla scala standard sopra. Il body text passa da `0.85-0.9rem` (13-14px) a un chiaro `text-sm` (14px), i titoli da `1.1rem` a `text-lg` (18px).
...
|---------------------|---------------------------------|----------------|
| `.status-generated` | `rgba(59, 130, 246, 0.15)`     | `#3b82f6`      |
| `.status-approved`  | `rgba(34, 197, 94, 0.15)`      | `#22c55e`      |
| `.status-rejected`  | `rgba(239, 68, 68, 0.15)`      | `#ef4444`      |
| `.status-revision`  | `rgba(234, 179, 8, 0.15)`      | `#eab308`      |
| `.status-published` | `rgba(168, 85, 247, 0.15)`     | `#1e5c7a`      |
| `.status-copied`    | `rgba(78, 205, 196, 0.15)`     | `#2d7d9a`      |
| `.status-archived`  | `rgba(150, 150, 150, 0.15)`    | `#999`         |
| `.status-review`    | `rgba(59, 130, 246, 0.15)`     | `#3b82f6`      |
...
> **DA MIGLIORARE rispetto a Content HUB**: il logo attualmente e "buttato li" con sfondo bianco. Adottare lo stile CashFlow.
...
**Esempio concreto - CashFlow attuale**:
- Icona: lettere "Kf" su sfondo scuro (`#0f172a`) con testo oro (`#d4af37`)
- Nome: "KW Cashflow"
- Versione: "v2.1"

**Esempio target - Content HUB**:
- Icona: icona Lucide `home` o `layout-dashboard` su sfondo scuro
- Nome: "KW Content Hub"
- Versione: "v1.5.0"
...
| *(nuova app)*| *(scegliere da Lucide)* | *(coerente col dominio)*   |

**Nota per il grafico**: NON servono piu asset PNG del logo nella sidebar. Si usano solo icone Lucide. Il logo PNG Karalisweb va usato solo nella **pagina di Login** (vedi sezione 6.1).
...
width: 20px;
height: 20px;
opacity: 0.7;      /* Inattivo */
opacity: 1;         /* Attivo */
```

**Le icone devono essere identiche in tutte le app.** Usare lo stesso set SVG inline stroke-based. Le icone delle 4 voci fisse (Profilo, Impostazioni, Guida, Esci) devono essere letteralmente le stesse in ogni app.
...
Lucide e una libreria open source con 1500+ icone, tutte **stroke-based** con stile uniforme. Questo garantisce che ogni icona abbia lo stesso spessore, le stesse proporzioni e lo stesso "peso visivo", indipendentemente dall'app in cui viene usata.

- **App React/Next.js** (es. CashFlow): usare il pacchetto `lucide-react`
- **App Vanilla JS** (es. Content HUB): usare le SVG di Lucide inline o tramite CDN
...
```html
<!-- Per progetti Vanilla JS - CDN -->
<script src="https://unpkg.com/lucide@latest"></script>
<script>lucide.createIcons();</script>

<!-- Oppure icone inline copiate da lucide.dev (viewBox 0 0 24 24) -->
```

**Content HUB attualmente** usa SVG inline custom. Queste vanno **sostituite** con le corrispondenti icone Lucide per allinearsi allo standard.
...
```css
/* Proprietà di base (automatiche con Lucide) */
...
Queste icone sono usate nei componenti comuni e **devono essere identiche** in tutte le app:
...
Oltre alle icone obbligatorie, ogni app usa icone specifiche per le proprie funzionalita.
...
| Progetti          | `folder`           |
| Collezioni        | `archive`          |
| Blog Analyzer     | `bar-chart-3`      |
| Genera            | `zap`              |
| Pubblica          | `upload-cloud`     |
...
| Previsionale       | `calendar-range`   |
| Piano Commerciale  | `receipt`          |
| Consuntivo         | `credit-card`      |
| Piani di Rientro   | `target`           |
| Piano Annuale      | `wallet`           |
| Notifiche          | `bell`             |
| Trend positivo     | `trending-up`      |
| Trend negativo     | `trending-down`    |
...
| Article item      | Border oro + background `--bg-secondary`            |
...
Tutte le app usano il formato **Semantic Versioning**: `vMAJOR.MINOR.PATCH` (es. `v1.5.0`)
...
| Punto                        | Esempio                        | Note                           |
|------------------------------|--------------------------------|--------------------------------|
| `package.json` → `version`  | `"version": "1.5.0"`          | Versione npm/node              |
| `deploy.sh` → `APP_VERSION` | `APP_VERSION="1.5.0"`         | Mostrata nel log di deploy     |
| `deploy.sh` → header commento | `Versione: 1.5.0`          | Documentazione dello script    |
| **Sidebar header** → versione | `v1.5.0`                     | Visibile all'utente nell'UI    |
| Config versione (se presente) | `version.js` o equivalente   | Per API o pagine info          |

> **Problema attuale**: Content HUB ha la versione disallineata tra `package.json` (1.5.0), `deploy.sh` (1.5.0) e `server/config/version.js` (1.4.3). Vanno sincronizzati.

### 13.3 Dove Mostrare la Versione nell'UI

La versione deve essere visibile in **2 punti**:

1. **Header sidebar** - sotto il nome dell'app (es. "v1.5.0" in `text-xs` colore `--text-muted`)
...
Il deploy script di **Content HUB** e il riferimento per la struttura (5 step, output colorato, verifica stato git). CashFlow va allineato a questo formato.
...
| File                          | Contenuto                      |
|-------------------------------|--------------------------------|
| `public/css/style.css`        | Stili principali dell'app      |
